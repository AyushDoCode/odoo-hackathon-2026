...
class AllocationService:
    ...
    async def allocate(self, data: AllocationCreate, *, created_by: UUID) -> Allocation:
        ...
        try:
            ...
            if current is not None and current.to_user_id is not None:
                holder = f"user {current.to_user_id}"
            elif current is not None and current.department_id is not None:
                holder = f"department {current.department_id}"
            else:
                holder = "another holder"
            raise AllocationError(
                f"Asset is already allocated (currently held by {holder}). "
