...
async def test_approve_sets_asset_maintenance_and_resolve_restores_available(
    ...
    try:
        ...
        assigned = await service.assign_technician(
            request.id, technician_id, actor_id=manager_id
        )
        assert assigned.status == MaintenanceStatus.TECHNICIAN_ASSIGNED

        in_progress = await service.start_progress(request.id, actor=technician)
        assert in_progress.status == MaintenanceStatus.IN_PROGRESS

        resolved = await service.resolve(request.id, actor=technician)
